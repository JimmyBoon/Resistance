...
namespace RPG.Dialogue.Editor
{
    public class DialogueEditor : EditorWindow
    {
        ...
        private void DrawLinkButtons(DialogueNode node)
        {
            ...
            else if (linkingParentNode.GetChildern().Contains(node.name))
            {
        ...
        private void DrawConnections(DialogueNode node)
        {
            ...
            foreach (DialogueNode childNode in selectedDialogue.GetAllChildern(node))
            {
                ...
                Vector3 controlPointOffest = endPosition - startPosition;
                controlPointOffest.y = 0;
                controlPointOffest.x *= 0.8f;

                Handles.DrawBezier(startPosition, endPosition, startPosition + controlPointOffest, endPosition - controlPointOffest, Color.white, null, 4f);
            }
        }
        ...
        private DialogueNode GetNodeAtPoint(Vector2 point)
        {
            ...
            foreach (DialogueNode node in selectedDialogue.GetAllNodes())
            {
                if (node.GetRect().Contains(scrollPosition + point))
                {
                    foundNode = node;
                }
            }
